...
// Bot privado, servidor único: esses caches ficam naturalmente limitados
// ao número de membros do servidor, então não precisam de faxina por tempo.
// A limpeza real acontece na saída do membro (guildMemberRemove no index.js),
// que chama a função abaixo.
function removerUsuarioDoCache(userId) {
  cachePerfilCompleto.delete(userId);
  cacheInacessivel.delete(userId);
}
...
function logarFormatoResposta(response) {
  ...
  try {
    console.log('[DEBUG bio] chaves da resposta:', Object.keys(response));
    ...
    console.log('[DEBUG bio] details:', JSON.stringify(response.details));
...
async function extrairTexto(response) {
  ...
  return String(response.response ?? response.body ?? response.data ?? '');
}
...
async function extrairJson(response) {
  ...
  if (!texto || !texto.trim()) {
    throw new Error('resposta vazia — não deu pra extrair JSON');
  }
...
function enfileirar(fn) {
  return new Promise((resolve, reject) => {
    filaPerfil.push({ fn, resolve, reject });
    processarFila();
  });
}
...
async function processarFila() {
  ...
  while (filaPerfil.length) {
    ...
    try {
      ...
      if (!token) {
        reject(new Error('todos os tokens estão mortos ou pausados'));
        continue;
      }
...
function obterToken() {
  ...
  for (const t of TOKENS) {
    ...
    if (e && e.vivo && Date.now() >= e.pausadoAte) {
      return t;
    }
  }
...
console.log(`[bio] ${TOKENS.length} tokens carregados (síncrono)`);
...
async function getUserBio(userId, guildId, { force = false } = {}) {
  if (!force) {
    const ultimo = await HistoricoBio.findOne({ userId })
      .sort({ registradoEm: -1 })
      .catch(() => null);
...
// ================================================================
async function getUserPerfil(userId, guildId /* agora pode ser null */, { force = false } = {}) {
  if (!force) {
  ...
  // se sabemos que esse usuário é inacessível, nem tenta de novo
  if (
  ...
  return enfileirar(async () => {
    ...
    try {
      ...
      // ====== daqui pra baixo é exatamente o seu código atual (429/403/401/400+) ======
      if (status === 429) {
